...
export function mediaUrl(media: Media | string | null | undefined, size: ImageSize = 'medium'): string | null {
  if (!media) return null
  if (typeof media === 'string') return media
  const fallback: ImageSize[] = ['large', 'medium', 'small', 'thumbnail']
  const order = size === 'original' ? fallback : [size, ...fallback.filter((s) => s !== size)]
